...
public class RedisService {
    private static final Long expire = 2L;
    private static final String cacheKey = "dish_";
    @Autowired
    ...
    public Long getExpire(String key, TimeUnit unit) {
        if(null==key||null==unit){
            throw new CustomException("key or TomeUnit 不能为空");
        }
    ...
    public Boolean setifAbsen(String key,Object value,long time,TimeUnit unit){
        ...
        if(null==key||null==value||null==unit){
            throw new CustomException("key、value、unit都不能为空");
        }
...
//******************hash数据类型*********************
    ...
    public Long hIncrBy(String key,Object field,long increment){
        if (null==key||null==field){
            throw new CustomException("key or field 不能为空");
        }
    ...
    public Object getCache(Object key){
        log.info("从缓存中获取数据KEY值[{}]",key);
        if (key == null) {
            return null;
        }
        String redisCacheKey = getCacheKey(key);
        Object rawValue = this.get(redisCacheKey);
        if (rawValue == null) {
            return null;
        }
        return rawValue;
    }
    public Object putCache(Object key, Object value) throws CacheException {
        ...
        try {
            String redisCacheKey = getCacheKey(key);
            this.set(redisCacheKey, value != null ? value : null, expire, TimeUnit.HOURS);return value;
        } catch (Exception e) {
            throw new CacheException(e);
        }
    }
    public void removeCache(Object key) throws CacheException {
...
//        }
    ...
    public int sizeCache() {
        int result = 0;
        try {
            result = this.keys(cacheKey + "*").size();
        } catch (Exception e) {
            log.error("get keys error", e);
        }
        return result;
